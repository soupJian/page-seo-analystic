...
export interface ImageTableRef {
  getCurrentDisplayData: () => any[];
}

const ImageTable = forwardRef<ImageTableRef, ImageTableProps>(
  ({ imageInfo, onExport }, ref) => {
    ...
    // 获取当前表格显示的数据（考虑排序、过滤，但不考虑分页）
    const getCurrentDisplayData = () => {
      let filteredData = [...rows];

      // 应用过滤
      if (columnFilters.alt) {
        filteredData = filteredData.filter(record => {
          const empty =
            !record.alt || record.alt.trim() === "" || record.alt === "-";
          return columnFilters.alt?.includes(empty ? "empty" : "nonempty");
        });
      }

      if (columnFilters.format) {
        filteredData = filteredData.filter(record => {
          const format = record.format || "other";
          return columnFilters.format?.includes(format);
        });
      }

      // 应用排序
      if (sortInfo.field === "bytes" && sortInfo.order) {
        filteredData.sort((a, b) => {
          const va =
            typeof a.size === "number" && !Number.isNaN(a.size) ? a.size : -1;
          const vb =
            typeof b.size === "number" && !Number.isNaN(b.size) ? b.size : -1;
          return sortInfo.order === "ascend" ? va - vb : vb - va;
        });
      }
